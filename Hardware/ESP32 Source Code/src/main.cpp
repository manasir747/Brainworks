...
void parseHazardPacket(String packet) {
    ...
    for (int i=0; i<MAX_HAZARDS; i++) {
        if (hazards[i].valid && gpsDistanceMeters(lat, lon, hazards[i].latitude, hazards[i].longitude) < 5.0) {
            slot = i; break;
        }
    }
    if (slot == -1) {
...
void calculateV2X() {
    ...
    for (int i=0; i<MAX_REMOTE_VEHICLES; i++) {
        if (remoteVehicles[i].valid) {
            ...
            float dist = gpsDistanceMeters(vehicleLatitude, vehicleLongitude, remoteVehicles[i].latitude, remoteVehicles[i].longitude);
            if (dist < nearestVehicleDistance) nearestVehicleDistance = dist;
        }
    }

    for (int i=0; i<MAX_HAZARDS; i++) {
        if (hazards[i].valid) {
            float dist = gpsDistanceMeters(vehicleLatitude, vehicleLongitude, hazards[i].latitude, hazards[i].longitude);
            dist = max(0.0f, dist - hazards[i].radius);
            if (dist < nearestHazardDistance) nearestHazardDistance = dist;
        }
    }

    v2xAlert = (nearestVehicleDistance <= V2X_WARNING_MAX && nearestVehicleDistance > V2X_WARNING_MIN);
}
...
void calculateTTC() {
    ...
    for (int i=0; i<MAX_REMOTE_VEHICLES; i++) {
        if (remoteVehicles[i].valid) {
            float dist = gpsDistanceMeters(vehicleLatitude, vehicleLongitude, remoteVehicles[i].latitude, remoteVehicles[i].longitude);
            if (abs(dist - nearestVehicleDistance) < 1.0) {
...
void sendTelemetry() {
    ...
    String packet = VEHICLE_ID + "," + VEHICLE_TYPE + "," + String(vehicleLatitude, 6) + "," + 
                    String(vehicleLongitude, 6) + "," + String(vehicleSpeedKmh, 1) + "," + String(vehicleHeading, 1);
    LoRa.beginPacket();
    LoRa.print(packet);
    LoRa.endPacket();
}
...
void sendRockfall(float lat, float lon, float radius) {
    ...
    String packet = "HAZARD,ROCKFALL," + String(lat, 6) + "," + String(lon, 6) + "," + String(radius, 1);
    LoRa.beginPacket();
    LoRa.print(packet);
    LoRa.endPacket();
...
void processSerialCommands() {
    ...
    if (cmd == "STATUS") {
        ...
        Serial.println("[SAFETY] Navigation unchanged. Vehicle direction unchanged.");
    } 
...
void printStatus() {
    ...
    Serial.println("GPS: " + String(gps.location.isValid() ? "VALID" : "NO FIX"));
    Serial.println("LoRa: " + String(loraHealthy ? "ONLINE" : "FAILED"));
    Serial.println("Speed: " + String(vehicleSpeedKmh, 1) + " km/h");
    Serial.println("Heading: " + String(vehicleHeading, 1) + " deg");
    Serial.println("Radar: " + String(radarDistance, 1) + " m");
    Serial.println("Nearest V2V: " + String(nearestVehicleDistance, 1) + " m");
    Serial.println("Nearest Hazard: " + String(nearestHazardDistance, 1) + " m");
    Serial.println("TTC: " + String(TTC > 999 ? 9999.0 : TTC, 1) + " s");
    Serial.println("V2X: " + String(v2xAlert ? "WARNING" : "CLEAR"));
